...
export default function Inventory() {
  ...
  const [newItem, setNewItem] = useState({ name: "", categoryId: "", currentStock: "", unit: "pcs", minStockLevel: "", usedInItems: "", costPerUnit: "" });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewItem({ name: "", categoryId: "", currentStock: "", unit: "pcs", minStockLevel: "", usedInItems: "", costPerUnit: "" });
      toast({ title: "Added", description: "Inventory item added" });
    },
  });

  const isLow = (item: InventoryItem) => item.currentStock <= item.minStockLevel;
  ...
  const handleSaveEdit = () => {
    ...
    updateMutation.mutate({
      id: editItem.id,
      data: { currentStock: parseInt(editQty), minStockLevel: parseInt(editMin) },
    });
  };

  const handleCreate = () => {
    if (!newItem.name || !newItem.categoryId || !newItem.currentStock || !newItem.minStockLevel) {
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map(item => {
                ...
                const pct = Math.min(100, Math.round((item.currentStock / (item.minStockLevel * 2 || 1)) * 100)); // Fixed typo
                return (
                  <div
                    key={item.id}
                    className={cn(
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <button
                        onClick={() => { setEditItem(item); setEditQty(String(item.currentStock)); setEditMin(String(item.minStockLevel)); }}
                    ...
                    <div className="flex items-baseline gap-1 mb-2">
                      ...
                      <span className="text-xs text-gray-300 ml-1">/ min {item.minStockLevel}</span>
                    </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" value={newItem.currentStock} onChange={e => setNewItem(p => ({ ...p, currentStock: e.target.value }))} className="rounded-xl border-gray-200" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5 block">Min Qty *</label>
                  <Input type="number" value={newItem.minStockLevel} onChange={e => setNewItem(p => ({ ...p, minStockLevel: e.target.value }))} className="rounded-xl border-gray-200" />
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
